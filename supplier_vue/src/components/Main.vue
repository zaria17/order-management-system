  ...
  <el-scrollbar>
    <div>
      <div style="margin-bottom:5px ;margin-top: 5px;margin-left: 5px">
        ...
        <el-button size="small" type="success" style="margin-left: 5px" @click="loadPost2">查看你的供应产品</el-button>
      </div>
  ...
  methods: {
    ...
    save() {
      this.$refs.form.validate(valid => {
        this.form.供应商id=this.user.供应商id
        if (valid) {
          if (this.form.产品id) {
            console.log(222)
            this.doMod();
    ...
    loadPost() {
      this.$axios.post('http://localhost:8090/产品/listP5', {
            "产品名称": this.产品名称,
            "类别id":this.类别id,
    ...
    loadPost2() {
      this.$axios.post('http://localhost:8090/产品/listP3', {
            //"订单id":this.订单id,
            "供应商id":this.user.供应商id
          }
      ).then(res => res.data).then(res => res.data).then(res => {
