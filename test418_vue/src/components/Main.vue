  ...
  <el-scrollbar>
    <div>
      <div style="margin-bottom:5px ;margin-top: 5px;margin-left: 5px">
        ...
        <el-button type="primary" style="margin-left: 5px" @click="addOrder">新建订单</el-button>
        <el-button :plain="true" type="primary" style="margin-left: 5px" @click="open1">查看当前时间</el-button>
        <img :src="imageSrc" alt="Image" style="margin-left: 15px;width:100px;height: 100px" />
      </div>
...
<!--        <el-table-column prop="再订购量" label="再订购量" />-->
        <el-table-column prop="operate" label="操作" width="100">
          <template slot-scope="scope">
            <el-button size="small" type="success" style="margin-left: 5px" @click="addToOrder(scope.row)">购买</el-button>
          </template>
        </el-table-column>
      </el-table>
      ...
      <el-dialog
        ...
        <el-form ref="form2" :model="form2" label-width="80px">
          <el-form-item label="订购日期" >
            <el-input v-model="form2.订购日期"></el-input>
          </el-form-item>
        </el-form>
        <el-form ref="form2" :model="form2" label-width="80px">
        ...
        <el-form ref="form2" :model="form2" label-width="80px">
          <el-form-item label="货主地址">
            <el-input v-model="form2.货主地址"></el-input>
          </el-form-item>
        </el-form>
        <el-form ref="form2" :model="form2" label-width="80px">
          <el-form-item label="货主城市">
            <el-input v-model="form2.货主城市"></el-input>
          </el-form-item>
        </el-form>
        <el-form ref="form2" :model="form2" label-width="80px">
          <el-form-item label="货主地区">
            <el-input v-model="form2.货主地区"></el-input>
          </el-form-item>
        </el-form>
        <span slot="footer" class="dialog-footer">
...
export default {
  ...
  methods: {
    ...
    addOrder(){
      ...
      this.form2.订购日期=this.currentTime
      console.log(this.form2.订购日期)
      this.$nextTick(() => {
        this.resetForm2();
        this.form2.订购日期=this.currentTime
      })
    ...
    save3(){
      this.$refs.form3.validate(valid => {
        if (valid) {
          ...
          if (this.form3.订单id) {
            console.log(222)
            if(this.form3.数量>this.form3.库存量) {
              console.log('error submit!!!');
              this.$message({
                message: '操作失败！',
                type: 'error'
    ...
    .catch(error => {
        ...
        // 在这里执行类似于上面提到的另一个 axios.post 请求来处理错误响应
    ...
    addToOrder(row){
      ...
      this.$nextTick(() => {
        ...
        this.form3.库存量=row.库存量
        console.log(this.form3.库存量)
    ...
    loadPost() {
      this.$axios.post('http://localhost:8090/产品/listP4', {
            "产品名称": this.产品名称,
            "类别id":this.类别id,
            "中止":false
          }
      ).then(res => res.data).then(res => res.data).then(res => {
        console.log(this.类别id)
        if (this.类别id>0){
          this.putimg();
        }
        this.tableData = res
